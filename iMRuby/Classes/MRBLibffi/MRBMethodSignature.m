...
@implementation MRBMethodSignature {
    NSString *_typeNames;
...
- (void)_parse
{
    ...
    for (int i = 0; i < _types.length; i ++) {
        ...
        if (c == '^') {
            ...
            // fix CGRect struct
            if ([[_types substringFromIndex:i] hasPrefix:@"{CGRect"]) {
                end = [[_types substringFromIndex:i] rangeOfString:@"}}"].location + i + 1;
            }
...
- (void)_parseTypeNames
{
    ...
    for (NSInteger i = 0; i < typeArr.count; i++) {
        ...
        int length = [MRBMethodSignature typeLengthWithTypeName:typeStr];
    ...
    switch (c[0]) {
        ...
        case '{':
        {
            ...
            if (end != NSNotFound) {
                ...
                NSDictionary *structDefine = registeredStruct[structName];
